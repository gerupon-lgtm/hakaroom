...
- 現在版: `hakaroom-0.9.0`（正典は `src/version.ts`。この節の値は更新を忘れがちなので、変更時は必ず両方直す）
...
| 横持ち（screen.orientation.angle=90、ロール≈+90°）で撮った写真は景色が正立して保存される。v0.8.2までの `upVectorInCamera` は回転の符号が逆で、鉛直ガイド線が実際の柱と逆に傾き、方式Aの距離が大きく縮んでいた（基準0.91mで1.42m→0.448m） | 実機スクリーンショット（床の写真、仰角-25.7°）のガイド線の向きと、読み取った点での再計算（符号を直すと1.42m→約1.6m） | ユーザー報告＋再計算。v0.8.3で修正（修正後の実機確認は未） | 2026-09-26 |
| 事実 | 検証方法 | 根拠 | 確認日 |
|------|----------|------|--------|
| （未実施） | — | — | — |
